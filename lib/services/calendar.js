...
const calendarService = {
   ...
   * @param {string} calendarSid - calendar sid under which seats are added
   * @param {number} data - Required data for removing seat with calendar seat
    ...
    return appointmentService
      ...
      .then((appointmentData) => {
        ...
        if (deletionList.length >= numberOfSeatsToRemove) {
          ...
          data.updateList = appointmentSeatSids;
          data.deletionList = deletionList;
          data.payload.seatSid = null;
          promises.push(seatService.deleteSeat(data));
          promises.push(calendarSeatService.deletecalendarSeat(data));
          promises.push(appointmentService.updateAppointment(data));
        }
   ...
   * @param {Object} syncOptions.currentNumberOfSeats - Current number of seats
   * @param {Object} syncOptions.requiredNumberOfSeats - Required number of seats
   * @returns {Proise} - Resolves with synced data
   */
  syncNumberOfSeats(syncOptions) {
    ...
    if (currentNumberOfSeats <= requiredNumberOfSeats) {
      syncOptions.payload.numberOfSeats = requiredNumberOfSeats - currentNumberOfSeats;
      return _this.addSeatsForCalendar(syncOptions.calendarSid, syncOptions);
    } else {
      syncOptions.payload.numberOfSeats = currentNumberOfSeats - requiredNumberOfSeats;
