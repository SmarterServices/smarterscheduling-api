...
JOIN salesforce.sc_calendar__c CL ON
  CL.sid__c = :calendarSid
JOIN salesforce.sc_calendar_seat__c CS ON
  CL.sid__c = CS.calendar__r__sid__c
JOIN salesforce.sc_seat__c SE ON
  CS.seat__r__sid__c = SE.sid__c
JOIN salesforce.sc_schedule__c SS ON
  SE.schedule__r__sid__c = SS.sid__c
  OR CL.sid__c = SS.calendar__r__sid__c
LEFT JOIN salesforce.sc_availability__c AV ON
  (extract(dow
from
  date_time) = AV.day_of_week__c)
  AND ((AV.start_date__c is null)
  OR (AV.start_date__c is not null
  AND date_time between AV.start_date__c AND AV.end_date__c))
