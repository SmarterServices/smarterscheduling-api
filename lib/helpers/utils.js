...
const utils = {
   ...
   * @param {boolean} [options.isTimestamp] - is the value a timestamp value
   * @param {boolean} [options.isTime] - is the value a time value
   * @returns {string} - Query string for update
   */
  getQueryCase: function (identifier, value, replacement, options = {}) {
    const isTimestamp = options.isTimestamp || false;
    const isTime = options.isTime || false;
  ...
  /**
   * Pick properties of a given object ignoring the depth
   * @param {Object} source - Source to pick properties from
   * @param {Array} propertiesToPick - Properties to pick form source
   * @returns {Object} - Object built form target properties
   */
  pickProperties(source, propertiesToPick) {
    const properties = {};

    propertiesToPick.forEach(propertyPath => {
      const propertyParts = propertyPath.split('.');
      const propertyKey = propertyParts[propertyParts.length - 1];

      properties[propertyKey] = _.get(source, propertyPath);
    });
  ...
  /**
   * Validates the sid sent with the payload
   * @param {string} sid - The sid which needs to be validated
   * @param {Function} getterService - The getter function which gets the information of the sid
   * @returns {*} - Rejects with error if that sid is found
   */
  isSidAvailable(sid, getterService) {
    //Validate the arguments
    try {
      utils.validateArguments(arguments, ['string', 'function.required']);
    } catch (error) {
      return Promise.reject(error);
    }

    // If no sid is sent with payload then resolves for usual post flow
    if (!sid) {
    ...
    return new Promise((resolve, reject) => {
      getterService({sid})
        .then(() => {
          ...
          reject(errorResponse.formatError('SID_ALREADY_EXISTS', errorOptions));
        })
        .catch((error) => {
