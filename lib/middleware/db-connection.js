...
function filterDBErrors(error) {
  //Possbly will be replaced with launch darkly
...
if (process.env.NODE_ENV !== 'dev-test' && process.env.DATABASE_URL) {
  _.set(dbConnectionConfig, 'databases[0].uri', process.env.DATABASE_URL);
}
