...
const calendarHandler = {
  ...
  addCalendar(data, callback) {
    let addedCalendar;
    let numberOfSeats;
    const propertiesToPick = ['params.accountSid', 'params.locationSid', 'payload', 'paramValidationResult'];
          ...
          .transaction(() => {
            return calendarService
              ...
              .then((calendarData) => {
                ...
                options.calendarSid = calendarData.sid;

                return calendarService.addSeatsForCalendar(options);
              })
              .then((seats) => {
                numberOfSeats = seats.results.length;
                options.payload.calendarSid = addedCalendar.sid;
                return scheduleService.addSchedule(options);
              });
          });
      })
    ...
    calendarService
      ...
      .then(() => {
        return sequelize.transaction(() => {
          const promises = [
            calendarService.updateCalendar(options),
            calendarSeatService.listCalendarSeat(options),
            scheduleService.updateSchedule(options)
          ];

          return utils
            .wrapValidation(promises)
            .then((returnedData) => {
              options.calendarSeats = returnedData[1].results;
              return calendarService.syncNumberOfSeats(options);
            });
        });
      })
      .then(() => {
